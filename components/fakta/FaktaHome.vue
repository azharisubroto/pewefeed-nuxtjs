<template>
	<section>
		<!-- SLIDER -->
		<v-skeleton-loader
			v-if="hero_is_fetching"
			class="mx-auto mt-5"
			type="image"
		></v-skeleton-loader>
		<HeroSlide
			v-if="hero_items.length > 0 && !hero_is_fetching"
			:items="hero_items"
		/>
		<!-- END SLIDER -->
		...
		<v-container>
			<v-row class="minheight">
				<v-col
					cols="12"
					v-if="latests.length > 0 && !latests_is_loading"
				>
					<NewsLoop
						:items="latests"
...
export default {
	...
	data() {
		return {
			...
			dataTitle:
				"Tonton Video FAKTA, Kumpulin Poinnya, Dapetin Hadiahnya! - Playworld",
