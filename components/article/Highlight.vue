<template>
	<div>
		...
		<div v-if="articles.length > 0">
			<v-container>
				<strong class="title"
					>{{ totalArticles }} Artikel Ditemukan</strong
				>
		...
		<div v-else class="text-center mt-10">
			<strong class="grey--text"
				>Maaf artikel yang anda cari tidak ditemukan.</strong
			>
		</div>
	</div>
</template>
...
export default {
	...
	methods: {
		async search() {
			try {
				const res = await ArticleService.searchHighlight(
		...
		async loadMore(n) {
			try {
				const res = await ArticleService.searchHighlightMore(
					this.$route.params.keywords,
					n
				)
