...
export default {
	...
	head() {
		return {
			title:
				"Baca berita " + this.$route.params.cat + " bisa dapet hadiah",
		}
	},
	data() {
		return {
			...
			dataTitle:
				...
				", Kumpulin Poinnya, Dapetin Hadiahnya! - Playworld",
	...
	methods: {
		async fetchArticle() {
			try {
				const res = await ArticleService.getListArticleByCategory(
		...
		async fetchTrending() {
			try {
				const res = await ArticleService.getTrendingArticleByCategory(
					this.$route.params.cat
				)
				// //console.log(JSON.parse(JSON.stringify(res.data.data)))
