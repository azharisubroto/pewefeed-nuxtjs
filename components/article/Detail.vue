	...
	<section>
		...
		<v-container class="mb-5 pb-5 articlewrapper">
			<v-row>
				<v-col>
					<!-- TITLE -->
					<h1 class="mb-0 text-27 lh-37 text-center maintitle">
						{{ respon.article.title }}
					</h1>
					...
					<div class="mt-4 text-12 text-center">
						...
						{{
							respon.article.total_comment
								? respon.article.total_comment
								: 0
						}}
						Komentar &bull;
						{{
							respon.article.total_quiz
								? respon.article.total_quiz
								: 0
						}}
						Kuis
				...
				<v-row class="pb-5">
					<v-col cols="12">
						<div class="article-thumb">
							<div
								class="partnership"
								v-if="respon.article.partnership != null"
							>
								In Partnership with
								{{ respon.article.partnership.title }}
								<img
									:src="respon.article.partnership.avatar"
									width="20"
									alt
								/>
							</div>
							<v-img
								:src="respon.article.image.small"
								:aspect-ratio="4 / 3"
              ...
              <RedeemCard class="mb-5" />
						...
						<div class="mt-4">
							<span class="grey--text caption"
								>Tulisan ini dibuat oleh tim
								{{ domainTitle }} dari berbagai sumber</span
							>
							<br />
							<v-row
				...
				>
					<v-tab-item value="kasihkomen">
						<v-alert class="mt-4" color="#0057FF" prominent>
							...
							Dapatkan 5 Poin atas setiap komentar dengan minimum
							20 kata
						</v-alert>
						<!-- TEXT AREA -->
						<v-textarea
					...
					>
						...
						>
							...
							>
								...
								Dapatkan 20 Points jika seluruh jawaban kamu
								benar
							</v-alert>
							<div
								v-if="
		...
		<client-only>
			<LoginModal
				:redirect="dataUrl"
				:dialogVisible="loginModalVisible"
				@close="myDialogClose"
			/>
			...
			>
				...
				>
					...
					<img
						:src="
							active_tab == 0
								? '/img/icons/articles/sedangdibaca-o.svg'
								: '/img/icons/articles/sedangdibaca-w.svg'
						"
						class="mb-1 d-block"
						width="20"
						height="20"
				...
				<v-btn
					class="tidakbolehaktif"
					v-else
					@click="recaptchalikemodal = true"
				>
					<span class="text-10">
						Menyukai
						<br />(+1 Poin)
				...
				>
					...
					<img
						:src="
							active_tab == 3
								? '/img/icons/articles/kuis-o.svg'
								: '/img/icons/articles/kuis-w.svg'
						"
						class="d-block"
			...
			<NotVip
				:dialogVisible="notVipDialogVisible"
				@close="myDialogClose"
			/>
			...
			<!-- RECAPTCHA LIKE -->
			<v-bottom-sheet
				v-if="recaptchalikemodal"
				v-model="recaptchalikemodal"
			>
				<v-sheet height="100%" color="transparent">
					<v-card style="border-radius: 0 !important">
						<v-toolbar
...
export default {
	...
	methods: {
		...
		async checkQuizStatus() {
			try {
				const res = await ArticleService.checkAnswered(
					"article",
					this.$route.params.articleslug
				)
	...
	watch: {
		comment_message: function (newval, oldval) {
			if (newval != oldval) {
				setTimeout(() => {
					var regex = /\s+/gi
					var words = newval.trim().replace(regex, " ").split(" ")
						.length
