<template>
	<div>
		...
		>
			...
			<v-tab href="#tab-1"> VIDEOS </v-tab>

			<v-tab href="#tab-2"> UPLOAD </v-tab>
		...
		<v-tabs-items v-model="tab" class="adjusted-tab-items pb-5">
				...
				<template v-if="video_latest">
					<v-container>
						...
						>
							...
							masing-masing konten yang diupload peserta
						</v-alert>
						...
						<v-row
							...
							v-if="band"
							@click="
								$router.push('/starx/band/detail/' + band.slug)
							"
						>
							<v-col cols="2">
								<v-avatar size="50">
						...
						<div v-for="latest in latests" :key="latest.id">
							<!-- <pre>{{latest}}</pre> -->
						...
						<div v-if="finalists.length == 0" class="py-3 caption">
							No Video
						</div>
						...
						<div v-if="winners.length == 0" class="py-3 caption">
							No Video
						</div>
					</v-container>
						...
						<v-col cols="12" md="12">
							<v-card max-width="375" class="mx-auto mt-5">
								...
								>
									<v-row class="fill-height">
										<v-card-title
											class="white--text mb-12 pl-12"
										>
											<div class="display-1 pl-12">
												{{ band.band_name }}
											</div>
										</v-card-title>
									</v-row>
									...
									<v-list-item>
										...
										<v-list-item-content>
											...
											<v-list-item-subtitle
												>Nama Band</v-list-item-subtitle
											>
										</v-list-item-content>
									</v-list-item>
			...
			<v-tab-item value="tab-3">
				<v-carousel height="300" :aspect-ratio="16 / 9" hide-delimiters>
					<v-carousel-item
...
export default {
	...
	methods: {
		...
		async getBandDetail() {
			try {
				const response = await StarxService.BandDetail(
					this.$route.params.bandslug
				)
				...
				if (response.data.data.band.avatar) {
					...
					this.avatar =
						"https://be3ad46f1850a93a8329-aa7428b954372836cd8898750ce2dd71.ssl.cf6.rackcdn.com/assets/frontend/img/program/placeholder-big.jpg"
				}
				...
				try {
					...
					if (res.data.status == 200) {
						...
						this.responsemessage =
							"Selamat! anda mendapat extra poin " +
							res.data.static_point +
							" poin"
						var current = document.getElementById("starcount-" + id)
							.textContent
						...
						this.snacksrc = ""
						this.responsemessage =
							"Anda sudah memberi star video ini"
					}
				} catch (error) {
					console.log(error)
				}
