			...
			<v-row>
				<v-col cols="12" md="4">
					<v-alert
						...
						"
					>
						Lengkapi BAND PROFILE dan MEMBER PROFILE
						...
						<v-expansion-panel>
							...
							<v-expansion-panel-content class="pt-3">
								...
								<div v-if="bandData.avatarband">
									<v-img
										:aspect-ratio="16 / 9"
										:src="bandData.avatarband"
									></v-img>
									<v-btn
										@click="bandData.avatarband = null"
										class="mt-2 grey lighten-1"
										depressed
										block
										dark
										color="grey"
				...
				<v-col cols="12" md="12" class="mt-0">
					<h4 class="pwhead text-uppercase">
					...
					<v-expansion-panels class="mb-3" focusable>
						...
						>
							<v-expansion-panel-header>
								<v-row no-gutters>
									<v-col cols="4">{{
										bandData.nama_personil[i]
											? bandData.nama_personil[i]
											: "NONE"
									}}</v-col>
									<v-col cols="8 text--secondary">{{
										bandData.personil_posisi[i]
											? getPosisi(
													bandData.personil_posisi[i]
											  )
											: "NONE"
			...
			try {
				...
				if (band) {
					...
					for (var i = 0, j = band_detail.length; i < j; ++i) {
						vm.bandData.nama_personil.push(
							Vue.util.extend(band_detail[i].personnel_name)
						)
						vm.bandData.instagram.push(
							Vue.util.extend(band_detail[i].instagram)
						)
						vm.bandData.avatar.push(
							Vue.util.extend(band_detail[i].avatar)
						)
						vm.bandData.personil_posisi.push(
							Vue.util.extend(band_detail[i].position.id)
						)
					}
				}
			} catch (error) {
