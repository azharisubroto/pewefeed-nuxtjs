				...
				<v-row class="pb-5">
					<v-col cols="12">
						...
						<div class="content" v-else>
							<v-card
								max-width="444"
								class="mx-auto grey lighten-4 pt-5"
								:elevation="0"
							>
								<flickity
									v-if="dataImage"
								...
								<v-row no-gutters class="mt-4">
									...
									>
									<v-col
										class="text-center orange pt-4 pb-4 white--text"
										cols="6"
									>
										<a
					...
					<template v-for="(rating, index) in ratings">
						<v-list-item :key="rating.customer.name + '-' + index">
							...
							<v-list-item-content>
								...
								<v-list-item-subtitle
									class="mt-1"
									v-html="rating.comment"
								></v-list-item-subtitle>
								<v-list-item-subtitle class="mt-1">
									<div
										class="d-inline-block mr-3 grey--text text--small"
									>
										<v-icon small size="12"
...
export default {
	...
	methods: {
		...
		async fetchImage() {
			try {
				const res = await ComicService.getImages(
					this.$route.params.detail
				)
		...
		async fetchContent() {
			try {
				let res = await ComicService.getDetail(
					this.$route.params.detail
				)
				// //console.log(JSON.parse(JSON.stringify(res.data.data)))
		...
		async fetchRating() {
			try {
				let res = await ComicService.getRating(
					this.$route.params.detail
				)
		...
		async submitRating() {
			...
			try {
				...
				vm.snackbar = true
				vm.responsemessage =
					"Maaf, Kamu hanya dapat memberi RATING satu kali."
			}
		},
