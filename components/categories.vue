		...
		<client-only>
			...
			<v-container>
				...
				<template v-if="highlights != null">
					<v-btn
						...
						:key="i"
						@click="
							$router.push('/highlight/' + highlight.title)
							drawer = false
						"
						depressed
						rounded
			...
			<template v-for="(cat, i) in categories">
				...
				<div cols="12" class="px-4" :key="'cat-' + i">
					<a class="pl-0" text dark :href="cat.link">{{
						cat.title
					}}</a>
				</div>
			...
			<template v-for="(program, i) in programs">
				...
				<div class="px-4" :key="'program-' + i">
					<a class="pl-0 d-block" text dark :href="program.link">{{
						program.title
					}}</a>
				</div>
			...
			<template v-for="(kelas, i) in classes">
				<div
					class="devider-small my-4"
					:key="'kelas-' + kelas.link"
				></div>
				<div class="px-4" :key="'kelas-' + i">
					<a class="pl-0 d-block" text dark :href="kelas.link">{{
						kelas.title
					}}</a>
				</div>
				<div
			...
			<!-- SOCIAL -->
			<v-container
				class="d-none text-center social justify-space-between"
			>
				<a
					target="blank"
