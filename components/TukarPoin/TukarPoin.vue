	...
	<section class="pb-5">
				...
				<div class="slidewrapper mb-0">
					...
					>
						<div
							...
							@click="
								$router.push('/tukarpoin/redeem/' + item.id)
							"
						>
							<div
								class="pt-5 px-3"
								:style="
									'color:#000!important;background: radial-gradient(circle, ' +
									item.category.color_base_1 +
									' 0%, ' +
									item.category.color_base_2 +
									' 100%);'
								"
							>
								<v-row>
			...
			<template v-if="groupedItems != null">
				<template v-for="(item, i) in groupedItems">
					<div
						...
						:key="'sectiontp-' + i"
						:style="
							'height:auto;padding-bottom:30px;color:#000!important;background: radial-gradient(at top left, ' +
							item.attributes.color_base_1 +
							' 0%, ' +
							item.attributes.color_base_2 +
							' 100%);overflow:hidden;'
						"
						class="py-5 mb-4"
					>
						<div class="plainslide-wrapper">
							<div class="plainslide" v-if="discounts">
								<div
									class="pt-4 text-18 lh-21 px-4"
									style="
										display: block;
										white-space: inherit;
										min-width: 123px;
								...
								<div
									...
									:class="[
										cool.expired == true ? 'expired' : '',
									]"
									style="min-width: 123px; max-width: 123px"
									@click="
										$router.push(
											'/tukarpoin/redeem/' + cool.id
										)
									"
								>
									<v-img
			...
			<section class="toppoin-acc" v-if="term">
				<v-expansion-panels v-for="(item, index) in term" :key="index">
					<v-expansion-panel class="mb-0">
						<v-expansion-panel-header class="py-5 text-uppercase">{{
							item.title
		...
		>
			...
			>
				<span>Rewards</span>
				<img
					src="/img/tukarpoin/tukarpoin-orange.png"
					class="mb-1 d-block"
					width="20"
					height="20"
				/>
