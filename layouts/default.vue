<template>
	<v-app :class="[drawer ? 'open' : 'closed']">
		...
		>
			...
			<v-tabs v-if="!$nuxt.$route.name.includes('member') && !$nuxt.$route.name.includes('starx') && !$nuxt.$route.name.includes('kompetisi') && !$nuxt.$route.name.includes('bantuan') && !$nuxt.$route.name.includes('c-codepw')" grow color="deep-orange" center-active class="pw-tab">
				<v-tab
