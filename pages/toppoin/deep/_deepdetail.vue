	...
	<section>
		...
		<section v-if="detail != null && isloading == false">
			...
			<div class="py-3">
				<div class="text-center py-4">
					Periode:
					{{
						[detail.periode.start_at, "YYYY-MM-DD HH:mm:ss"]
							| moment("DD MMM")
					}}
					-
					{{
						[detail.periode.end_at, "YYYY-MM-DD HH:mm:ss"]
							| moment("DD MMM YYYY")
					}}
				</div>
				<div></div>
				...
				<template>
						...
						<v-expansion-panels v-for="(item, i) in howto" :key="i">
							<v-expansion-panel class="mb-0">
								<v-expansion-panel-header
									...
									>{{ howto.title }}</v-expansion-panel-header
								>
					...
					<section class="toppoin-acc mainprizes">
						<v-expansion-panels>
							<v-expansion-panel>
								<v-expansion-panel-header
									>MAIN PRIZES</v-expansion-panel-header
								>
								<v-expansion-panel-content class="caption px-0">
									<template
										v-for="(item,
										i) in prizewithoutpemenang"
									>
										<template v-if="i < 3">
											<div
												class="py-0 px-4"
					...
					<section class="toppoin-acc mainprizes">
						<v-expansion-panels>
							<v-expansion-panel class="mb-6">
								<v-expansion-panel-header
									>OTHER PRIZES</v-expansion-panel-header
								>
								<v-expansion-panel-content class="caption px-0">
									<template
										v-for="(item,
		...
		>
			...
			<v-btn @click="$router.push('/toppoin')">
				<span>Ranking</span>
				<img
					src="/img/tukarpoin/icon-rank-orange.png"
					class="mb-1 d-block"
					width="20"
					height="20"
				/>
			...
			<v-btn @click="$router.push('/toppoin')">
				<span>Winners</span>
				<img
					src="/img/tukarpoin/icon-winner-orange.png"
					class="mb-1 d-block"
					width="20"
					height="20"
				/>
...
export default {
	...
	methods: {
		async fetchDetail() {
			try {
				const res = await TopPoin.periodeDetail(
					this.$route.params.deepdetail
				)
