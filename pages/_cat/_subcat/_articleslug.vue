...
export default {
	...
	async fetch({ store, params }) {
		...
		let item = await ArticleService.getDetail(params.articleslug).then(
			(res) => {
	...
	head() {
		...
		var url =
			...
			host +
			"/" +
			this.item.article.type.toLowerCase() +
			"/" +
			this.item.article.reaction.toLowerCase() +
			"/" +
			this.item.article.slug
		return {
			...
			meta: [
				{
					hid: "description",
					name: "description",
					content: this.item.article.short_content,
				},
				// Facebok
				{
					hid: "og:title",
					property: "og:title",
					content: this.item.article.title,
				},
				{
					hid: "og:description",
				...
				{ hid: "og:url", property: "og:url", content: url },
				{
					hid: "og:image",
					property: "og:image",
					content: this.item.article.image.medium,
				},
				{
					hid: "og:image:width",
