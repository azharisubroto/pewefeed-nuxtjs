  ...
  <div class="bantuan">
    <div v-if="menu && !loading">
      ...
      <v-tabs-items v-model="tab" class="djusted-tab-items pb-5">
        ...
        <v-tab-item v-for="item in menu" :key="'tabitem-'+item.id" :value="'tab-'+item.id">
          <v-list v-if="item.submenu">
            ...
            >
              <v-list-item-content>{{submenu.title}}</v-list-item-content>
              <v-list-item-icon>
    ...
    <div v-else class="text-xs-center" align="center">
      <v-progress-circular indeterminate :size="80" :width="8" color="deep-orange"></v-progress-circular>
    </div>
  </div>
</template>
<script>
