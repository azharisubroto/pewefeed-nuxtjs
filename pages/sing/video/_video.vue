<template>
	<div>
		<SingAppBar
			:title="content2 != null ? content2.stage : ''"
			:back="true"
		/>
		...
		<v-container class="hero-detailsing pb-5">
			...
			<!-- CONTENT CARD HEAD -->
			<v-card v-if="item" color="#404040" class="pt-3">
				<div
					v-if="content2 != null"
				...
				>
					...
					<div>
						<v-btn
							v-if="content2.customer.instagram != null"
							:href="
								'https://instagram.com/' +
								content2.customer.instagram
							"
							target="_BLANK"
							class="ml-2"
							small
				...
				<div class="d-flex justify-space-between pr-4 align-center">
					<div>
						<div
							class="text-20 float-left text-center px-4"
						...
						>
							<v-icon
								...
								>mdi-star</v-icon
							>
							<v-icon size="25" v-else color="#fff"
					...
					<div
						class="singmetaicons text-12 d-flex align-center justify-content-end metasing"
					>
						{{ content2.viewers }} Melihat &nbsp;&bull;&nbsp;
						{{ content2.total_vote ? content2.total_vote : "0" }}
						Vote &nbsp;&bull;&nbsp;
						{{
		...
		<template v-if="maintab == 0">
			...
			<template v-if="votetab == 0">
				<v-container class="pb-0">
					<v-alert class="mt-0 text-14" color="#0057FF" prominent>
						...
						Dapatkan 5 Poin atas setiap Vote. Vote pada
						masing-masing kontestan hanya bisa diberikan 1 kali per
						1 hari.
					</v-alert>
				</v-container>
				...
				<div v-if="voters.length > 0">
					<v-row no-gutters class="px-2">
						<template v-for="(voter, i) in voters">
							<v-col cols="6" class="pa-1" :key="'voter-' + i">
								<v-card
									color="#404040"
									dark
									elevation="0"
			...
			<template v-if="votetab == 1">
				<v-container>
					Untuk ikut mendukung kontestan favoritmu kebabak selanjutnya
					berikan vote dengan cara: <br /><br />
					...
					3. Satu Vote berlaku hanya berlaku untuk satu kontestan
					dalam satu hari <br />
					4. Jika Vote memenuhi syarat maka penyanyi favorit kamu bisa
		...
		<template v-if="maintab == 1">
			...
			<v-container v-if="comment_tab == 1">
				...
				<ol class="mb-5 pb-5">
					...
					<li>
						Poin hanya diberikan 1 kali untuk 1 User per 1 Artikel
					</li>
					<li>
						Seluruh komentar dimoderasi oleh tim Playworld ID dan
						bisa dihapus dan akan mengurangi total POIN jika
						komentar mengandung konten SARA, atau tidak sesuai
						dengan artikel yang dibaca
					</li>
				</ol>
			</v-container>
		</template>
		...
		>
			<v-btn @click="maintab = 0">
				...
				<img
					:src="
						singtab == 0
							? '/img/icons/vote-o.svg'
							: '/img/icons/vote-w.svg'
					"
					class="mb-1 d-block"
					width="20"
					height="20"
			...
			<v-btn @click="maintab = 1">
				...
				<img
					:src="
						singtab == 1
							? '/img/icons/articles/komentar-o.svg'
							: '/img/icons/articles/komentar-w.svg'
					"
					class="mb-1 d-block"
					height="20"
					alt
...
export default {
	...
	async fetch({ store, params }) {
		var item = await SingService.getDetailVideo(params.video).then(
			(res) => {
				return res.data
			}
		)
	...
	methods: {
		...
		async fetchComment() {
			try {
				const res = await SingService.getComments(
		...
		async loadVoters(n) {
			try {
				const res = await SingService.getVoters(
					this.$route.params.video,
					n
				)
