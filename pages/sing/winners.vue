<template>
	<section>
		...
		<v-container style="margin-top: -180px">
			...
			<template v-for="(item, i) in prizeswithpemenang">
				<v-card
					...
					:key="'winner-' + i"
					@click="
						$store.commit('SET_JUDGE_NUMBER', parseFloat(i + 1))
						$router.push('/sing/winner/' + item.winner_id)
					"
				>
					<div class="px-4 pt-4">
						<img src="/img/trophy.svg" alt="" width="16" />
						<strong>Juara {{ i + 1 }}</strong>
					</div>
					...
					<div class="px-4">
						<v-row>
							<v-col cols="4" class="px-0 pb-0">
								<img
									v-if="item.prize.redeem.image"
									:src="item.prize.redeem.image"
									alt=""
								/>
							</v-col>
		...
		<v-container class="pt-7"
			><h4 class="text-18">Prizes for Judges Votes</h4></v-container
		>
