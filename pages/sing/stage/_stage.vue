	...
	<section class="sing-stage">
		...
		<v-container class="pb-0" v-if="userid && lock && savemyitem != null">
			<template v-for="(item, i) in savemyitem">
				<div
					v-if="item.customer.id == userid"
					class="mx-0 px-0"
					:key="'peserta-' + i"
				>
					<v-card color="#FFC107" class="px-4 py-4" light>
		...
		<template v-if="!isloading">
			<template>
				<StageContent
					...
					:content="content"
					:pesertaloop="peserta"
					:stage="content.stage.id"
					title="STAGE 1: Audisi ini berakhir tanggal 30 Agustus 2020"
				/>

				<v-container
					v-if="peserta != null && peserta.length > 0"
				...
				>
					<v-btn
						...
						v-if="page < maxpage"
					>
						<template v-if="!moreloading"> Show More </template>
		...
		>
			<v-btn @click="sortopen = true">
				<span class="text-10" style="color: #fff !important"
					>Urutkan<br />Data</span
				>
				<img
				...
				<v-toolbar :elevation="1" style="border-top: 2px solid #404040">
					...
					<v-toolbar-items class="ml-3">
						<v-btn
							dark
							text
							class="white--text pl-0 text-uppercase"
				...
				<div class="mx-0 text-left px-4 pt-4">
					<template v-for="(item, i) in sorter">
						<v-btn
							...
							style="text-transform: normal !important"
							>{{ item.title }}</v-btn
						>
					</template>
			...
			<v-sheet height="100%">
				...
				<div class="px-5 pb-10 pt-6">
					...
					<div class="text-right">
						<v-btn
							:disabled="searchModel.length == 0"
							depressed
							@click="search(searchModel, 1, false)"
							color="deep-orange"
							class="mt-3"
							>Cari</v-btn
						>
	...
	methods: {
		async stageDetail(slug, page) {
			...
			try {
				...
				if (
					data.video_customer != null &&
					data.video_customer.length > 0
				) {
		...
		next(num) {
			...
			if (this.type == "search") {
				...
				this.sortItem(
					this.sorter[this.sorterMode]["slug"],
					pagenum,
					true
				)
			} else {
		...
		async search(key, page, ismore) {
			...
			try {
				const res = await SingService.searchItem(
		...
		async sortItem(key, page, ismore) {
			...
			try {
				const res = await SingService.sortStageItem(
					this.$route.params.stage,
					key,
					page
				)
		...
		gimanadah() {
			this.$bus.$emit("toggleupload", this.userid)
		},
	},
	mounted() {
		let vm = this
