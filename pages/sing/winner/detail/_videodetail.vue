	...
	<div>
		...
		<v-container class="hero-detailsing pb-10">
			...
			<div v-if="content2 != null" class="mt-4">
				...
				<div class="d-flex justify-space-between">
					...
					>
						<v-icon
							:color="content2.is_star ? '#FFC107' : '#f6f6f6'"
							>mdi-star</v-icon
						>
						Star
					</div>
					...
					<div
						class="singmetaicons text-14 d-flex align-center justify-content-end metasing"
					>
						...
						<div class="d-inline-block ml-3">
							<v-img
								src="/img/icons/comment-single.svg"
								width="14"
								max-width="14"
								class="mr-1 d-inline-block"
							></v-img>
						...
						>
							<div class="d-flex" style="width: 100%">
								...
								<div>
									<div class="text-14">
										<strong>{{
											voter.name ? voter.name : "null"
										}}</strong>
									</div>
									<div class="text-12">
										<strong>{{
											voter.date ? voter.date : "null"
										}}</strong>
									</div>
								</div>
							</div>
		...
		>
			<v-btn @click="maintab = 0">
				...
				<img
					src="/img/icons/icon-vote-orange.png"
					class="mb-1 d-block"
					width="20"
					height="20"
				/>
				...
				<v-toolbar :elevation="1" style="border-top: 2px solid #fff">
					...
					<v-toolbar-items>
						...
						>
							<span v-if="type == 'vote'"
								>You've got
								{{ type == "vote" ? 5 : 2 }} Point!</span
							>
							<span
								v-if="type == 'alreadyvote' || type == 'komen'"
...
export default {
	...
	async fetch({ store, params }) {
		var item = await SingService.getDetailVideo(params.videodetail).then(
			(res) => {
				return res.data
			}
		)
	...
	methods: {
		...
		async getVideoDetail(slug) {
			...
			try {
				const res = await SingService.winnerVideoDetail(slug)
				const data = res.data
